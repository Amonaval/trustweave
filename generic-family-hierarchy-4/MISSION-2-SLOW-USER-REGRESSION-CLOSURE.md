...
## QA harness correction after first broad runtime execution
The first broad staging run exposed repeated stale QA assumptions before many product flows could execute: cross-vertical suites expected the Family-only `qa-admin-center` marker, and older login paths depended on a single `qa-open-auth` state while Playwright could reuse a stale dev server. These are corrected in `MISSION-2-QA-HARNESS-CORRECTION.md`. Mission 2 must be rerun after this correction; the earlier 58 P1 findings are not individually classified as product defects until reproduced with the corrected harness.
