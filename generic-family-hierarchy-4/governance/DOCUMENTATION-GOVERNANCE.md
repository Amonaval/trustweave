...
- `governance/DOCUMENTATION-GOVERNANCE.md` — this authority model.
- `governance/DEVELOPMENT-RULES.md` — implementation/safety rules that are not constitution-level.
...
Historical root/compatibility documents belong under `history/root-legacy/`. Older mission evidence may remain under `archive/` when accepted legacy gates depend on those immutable paths. New historical root artifacts must not be added back to repository root.
...
The deterministic documentation gate enforces:
...
M3-C0 caps the root Markdown control surface at **10 files**. Root is reserved for bootstrap, constitutions, lifecycle and current descriptive truth.

- New mission records belong under `missions/`.
- Machine policy belongs under `governance/`.
- Product narrative belongs under `docs/product/`.
- Removed/superseded root Markdown belongs under `history/root-legacy/`.
- Existing `archive/` paths remain only where historical evidence/gates require compatibility.
- A new historical/apply/handoff Markdown file in root is a documentation-gate failure.
