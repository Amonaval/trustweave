...
const resultsRoot=process.env.QA_PLAYWRIGHT_RESULTS_DIR||'qa-results';
let browserArgs:string[]=[];try{browserArgs=JSON.parse(process.env.QA_BROWSER_ARGS_JSON||'[]')}catch{}
const launchOptions=process.env.QA_BROWSER_EXECUTABLE?{executablePath:process.env.QA_BROWSER_EXECUTABLE,args:browserArgs}:undefined;
...
export default defineConfig({
 ...
 testDir:'./qa/e2e',outputDir:path.join(resultsRoot,'artifacts'),fullyParallel:false,forbidOnly:!!process.env.CI,retries:process.env.CI?1:0,workers:1,timeout:90_000,expect:{timeout:10_000},
 reporter:[['list'],['html',{outputFolder:path.join(resultsRoot,'html'),open:'never'}],['junit',{outputFile:path.join(resultsRoot,'junit.xml')}],['json',{outputFile:path.join(resultsRoot,'playwright.json')}],['./qa/lib/bug-reporter.ts']],
 use:{baseURL,trace:'retain-on-failure',screenshot:'only-on-failure',video:'retain-on-failure',actionTimeout:15_000,navigationTimeout:45_000,launchOptions},
 projects:[
