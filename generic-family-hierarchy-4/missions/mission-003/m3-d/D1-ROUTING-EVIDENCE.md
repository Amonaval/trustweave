...
- Route unit contracts cover three representative verticals, every registered navigation ID, invalid/private URL tokens, cross-tenant and inactive membership, admin denial, legacy notification fallback and external href rejection. They are included in `npm run qa:unit` for CI regression protection.
- `npm run qa:m3d1:routing` exercises the real browser against the existing dedicated two-vertical staging seed: Family Community and Housing direct entry/refresh/history, sign-in return, plus the seed's isolated Family tenant for its own direct entry/refresh and cross-tenant denial. This command is available but was not run here because the attached archive has no staging QA environment or seed credentials.
