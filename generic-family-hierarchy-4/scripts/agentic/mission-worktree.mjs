import fs from 'node:fs';import path from 'node:path';import {execFileSync} from 'node:child_process';import {parseArgs,readJson,ROOT} from './lib.mjs';
const a=parseArgs();const missionPath=a.mission||'missions/mission-003/m3-b6-e1/mission.json';const m=readJson(missionPath);const branch=m.execution?.branch||`agent/${m.id.toLowerCase()}`;const rel=m.execution?.worktree||`.agent-worktrees/${m.id.toLowerCase()}`;const target=path.resolve(ROOT,rel);const base=m.execution?.baseRef||'main';
