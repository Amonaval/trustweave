...
const cli=process.argv.slice(2);const playwrightOnly=cli.includes('--playwright-only');if(cli.includes('--headed'))process.env.QA_M2_HEADED='true';const paceArg=cli.find(x=>x.startsWith('--pace='));if(paceArg){const pace=paceArg.split('=')[1];process.env.QA_USER_PACE_MS=pace;process.env.QA_CRAWL_PACE_MS=pace;}
process.env.QA_USER_PACE_MS=process.env.QA_USER_PACE_MS||'700';process.env.QA_CRAWL_PACE_MS=process.env.QA_CRAWL_PACE_MS||'700';process.env.QA_M2_CRAWL_ACTIONS=process.env.QA_M2_CRAWL_ACTIONS||'22';process.env.QA_M2_SERVER_WARMUP_MS=process.env.QA_M2_SERVER_WARMUP_MS||'50000';
const warmMarker='qa-results/mission2/.server-warmed';try{fs.rmSync(warmMarker,{force:true})}catch{}
const steps=[];
...
}else{steps.push({name:'source-and-build-preflight',status:'skipped',required:false,reason:'--playwright-only starts directly at browser runtime QA and does not run build/static/source gates.'});}
if(!isStaging()||!mutationAllowed()){
 ...
 else if(playwrightOnly){steps.push({name:'deterministic-qa-fixture',status:'blocked',required:true,reason:'--playwright-only requires existing qa-results/fixtures/seed-state.json. Run the full Mission 2 command once if the fixture is missing.'});}
