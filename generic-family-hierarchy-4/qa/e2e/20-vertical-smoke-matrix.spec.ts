...
test.describe('all released vertical × role runtime smoke matrix',()=>{
 for(const v of VERTICALS)for(const role of roles)test(`${v.label} [vertical:${v.kind}] [role:${role}] opens deterministic shell`,async({page})=>{
  ...
  const admin=page.getByTestId('qa-nav-admin');if(role==='member')await expect(admin).toHaveCount(0);else{await expect(admin).toHaveCount(1);await openSurface(page,'admin');await expect(page.getByTestId('qa-admin-center')).toBeVisible();}
 });
});
