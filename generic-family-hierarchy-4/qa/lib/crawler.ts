...
export async function expertCrawl(page:Page,testInfo:TestInfo,{maxActions=60,role='unknown',scope='app',expectedTestIds=[],timeBudgetMs=120_000,allowDestructive=false}:{maxActions?:number;role?:string;scope?:string;expectedTestIds?:string[];timeBudgetMs?:number;allowDestructive?:boolean}={}){
 ...
 for(let n=0;n<maxActions;n++){
  ...
  for(let i=0;i<count;i++){
   ...
   if(disabled||(!text&&!testId))continue;const label=testId||text;if(external.test(label)||external.test(href||''))continue;if(destructive.test(label)&&(!allowDestructive||!mutationAllowed()))continue;
 ...
 watch.flush();const dir=process.env.QA_CRAWL_EVIDENCE_DIR||'qa-results/crawl';fs.mkdirSync(dir,{recursive:true});const inventoryRows=[...nodes.values()];const discoveredIds=[...new Set(inventoryRows.map(x=>x.testId).filter(Boolean) as string[])].sort();const missingExpected=expectedTestIds.filter(x=>!discoveredIds.includes(x));const prefix=`${role}-${scope}`.replace(/[^a-z0-9-]+/gi,'-').toLowerCase();
