...
import {resolveNotificationDeepLink} from "../../lib/notification-routing";
...
export default function NotificationCenter({disabled=false}:{disabled?:boolean}){
 ...
 const [open,setOpen]=useState(false),[settingsOpen,setSettingsOpen]=useState(false),[showMuted,setShowMuted]=useState(false),[items,setItems]=useState<Notification[]>([]),[unread,setUnread]=useState(0),[busy,setBusy]=useState(false),[error,setError]=useState(""),[prefs,setPrefs]=useState<EngagementNotificationPreferences|null>(null),[pushBusy,setPushBusy]=useState<"enable"|"disable"|null>(null),[pushState,setPushState]=useState<{supported:boolean;permission:string;subscribed:boolean}>({supported:false,permission:"default",subscribed:false});
 ...
 const openItem=async(n:Notification)=>{setBusy(true);try{if(!n.read_at)await markNotificationRead(n.id);if(n.network_id)await setActiveNetwork(n.network_id);const href=resolveNotificationDeepLink(n);window.location.assign(href)}catch(e:any){setError(e.message||t("NotificationCouldNotOpenTxt"));setBusy(false)}};
 const togglePush=async()=>{
  const action=pushState.subscribed?"disable":"enable";setPushBusy(action);setBusy(true);setError("");
  try{
   if(pushState.subscribed)await disableWebPush();else await enableWebPush();
   const nextState=await getWebPushState();setPushState(nextState);
   const base=prefs||await fetchEngagementNotificationPreferences();
   const nextPrefs={...base,push_enabled:nextState.subscribed};
   await saveEngagementNotificationPreferences(nextPrefs);setPrefs(nextPrefs);
  }catch(e:any){setError(e.message||t("PushCouldNotChangeTxt"))}
  finally{setPushBusy(null);setBusy(false)}
 };
 ...
 const drawer=open&&typeof document!=="undefined"?createPortal(<div className="notification-drawer-backdrop" data-testid="qa-notification-drawer" onMouseDown={e=>{if(e.target===e.currentTarget)setOpen(false)}}><aside className="notification-drawer" role="dialog" aria-modal="true" aria-label={t("NotificationInboxTxt")}>
   <header><div><small>{t("YourUpdatesTxt")}</small><h2>{t("NotificationInboxTxt")}</h2></div><div><button className="icon-button" title={t("E10EngagementControlCenterTxt")} onClick={()=>setSettingsOpen(x=>!x)}><Settings2 size={18}/></button><button className="btn small notification-mark-all" disabled={busy||unread===0} title={t("MarkAllReadTxt")} aria-label={t("MarkAllReadTxt")} onClick={markAll}><CheckCheck size={18}/><span className="notification-action-label">{t("MarkAllReadTxt")}</span></button><button className="icon-button" aria-label={t("CloseTxt")} onClick={()=>setOpen(false)}><X size={18}/></button></div></header>
   {settingsOpen&&prefs&&<section className="e10-control-center" data-testid="qa-engagement-control-center"><div className="e10-control-head"><div><small>{t("E10CurrentNetworkTxt")}</small><h3>{t("E10EngagementControlCenterTxt")}</h3><p>{t("E10ControlCenterDescTxt")}</p></div></div>
    <div className="e10-device-row"><Smartphone size={18}/><span><b>{t("DeviceNotificationsTxt")}</b><small>{pushState.subscribed?t("PushEnabledTxt"):t("PushEnableHelpTxt")}</small></span>{pushState.supported?<button className="btn small" disabled={busy||!!pushBusy} onClick={()=>void togglePush()}>{pushBusy==="enable"?t("E10EnablingPushTxt"):pushBusy==="disable"?t("E10DisablingPushTxt"):pushState.subscribed?t("DisableTxt"):t("EnableTxt")}</button>:<em>{t("NotSupportedTxt")}</em>}</div>
   ...
   {!settingsOpen&&<div className="notification-push-control"><Smartphone size={17}/><span><b>{t("DeviceNotificationsTxt")}</b><small>{pushState.subscribed?t("PushEnabledTxt"):t("PushEnableHelpTxt")}</small></span>{pushState.supported?<button className="btn small" disabled={busy||!!pushBusy} onClick={()=>void togglePush()}>{pushBusy==="enable"?t("E10EnablingPushTxt"):pushBusy==="disable"?t("E10DisablingPushTxt"):pushState.subscribed?t("DisableTxt"):t("EnableTxt")}</button>:<em>{t("NotSupportedTxt")}</em>}</div>}
   {error&&<div className="notification-error">{error}</div>}{busy&&!pushBusy&&<div className="notification-busy"><LoaderCircle className="spin" size={18}/>{t("OpeningTxt")}</div>}
 ...
 return <><button data-testid="qa-notification-bell" className="notification-bell-button" title={t("NotificationInboxTxt")} aria-label={t("NotificationInboxTxt")} onClick={()=>{setOpen(true);void load()}}><Bell size={18}/>{unread>0&&<span>{unread>99?"99+":unread}</span>}</button>{drawer}</>;
}
